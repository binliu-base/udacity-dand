    ...
    if is_street_name(secondary):
        ...
        if m:
            ...
            if street_type not in street_expected:
                ...
                _,better_name = update_street_name(secondary.attrib['v'],all_mapping)
                new['value'] = better_name
            else:
        ...
        if city_name != city_expected: 
            if city_name not in city_mics:
                m = city_name_re.match(city_name)   
                if m:
                    _, better_name = update_city_name(city_name, city_mapping)
                    new['value'] = better_name
                else:
                    return None                
            else:
